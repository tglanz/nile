...
class Renderer {
 private:
  void initialize(RendererOptions& options) {
    ...
    std::vector<const char*> extensions(options.extensions.size() + 2);
    std::transform(options.extensions.begin(), options.extensions.end(), extensions.begin(), [](const std::string& str) {
      return str.c_str();
    });
    ...
    VkInstanceCreateInfo instanceCreateInfo = {
      ...
      .enabledExtensionCount = static_cast<uint32_t>(options.extensions.size()),
